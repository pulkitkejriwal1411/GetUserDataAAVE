...
const ProtocolDataContract = new web3.eth.Contract(ProtocolDataABI, ProtocolDataAddress);
...
describe("Resolver", function () {
    ...
    it('should return user token data',async function(){
        ...
        {
          const UserAssetData = await resolver.GetUserAssetData(userAddress,arr[i]);
          const userDataWeb3 = await ProtocolDataContract.methods.getUserReserveData(arr[i],userAddress).call();
          for(let j=0;j<9;j++)
          {
            expect(userDataWeb3[j].toString()).to.equal(UserAssetData[j].toString())
          }
        }
    }).timeout(300000);
});
...
async function GetUserDetails(){
    let userData = await LendingPoolcontract.methods.getUserAccountData(userAddress).call();
    return userData;
}
